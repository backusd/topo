...
GeometryGenerator::MeshData GeometryGenerator::CreateBox(float width, float height, float depth, uint32 numSubdivisions) noexcept
{
	MeshData meshData;
...
GeometryGenerator::MeshData GeometryGenerator::CreateSphere(float radius, uint32 sliceCount, uint32 stackCount) noexcept
{
	MeshData meshData;
...
void GeometryGenerator::Subdivide(MeshData& meshData) noexcept
{
	...
	{
		// NOTE: Cast to a uint64_t to avoid warning message about implicit 4-byte to 8-byte cast
		Vertex v0 = inputCopy.Vertices[inputCopy.Indices32[static_cast<uint64_t>(i) * 3 + 0]];
		Vertex v1 = inputCopy.Vertices[inputCopy.Indices32[static_cast<uint64_t>(i) * 3 + 1]];
		Vertex v2 = inputCopy.Vertices[inputCopy.Indices32[static_cast<uint64_t>(i) * 3 + 2]];
...
GeometryGenerator::Vertex GeometryGenerator::MidPoint(const Vertex& v0, const Vertex& v1) noexcept
{
...
GeometryGenerator::MeshData GeometryGenerator::CreateGeosphere(float radius, uint32 numSubdivisions) noexcept
{
	MeshData meshData;
...
GeometryGenerator::MeshData GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, uint32 sliceCount, uint32 stackCount) noexcept
{
	MeshData meshData;
...
GeometryGenerator::MeshData GeometryGenerator::CreateGrid(float width, float depth, uint32 m, uint32 n) noexcept
{
	MeshData meshData;
	...
	for (uint32 i = 0; i < m; ++i)
	{
		...
		for (uint32 j = 0; j < n; ++j)
		{
			...
			// NOTE: Cast to a uint64_t to avoid warning message about implicit 4-byte to 8-byte cast
			meshData.Vertices[static_cast<uint64_t>(i) * n + j].Position = XMFLOAT3(x, 0.0f, z);
			meshData.Vertices[static_cast<uint64_t>(i) * n + j].Normal = XMFLOAT3(0.0f, 1.0f, 0.0f);
			meshData.Vertices[static_cast<uint64_t>(i) * n + j].TangentU = XMFLOAT3(1.0f, 0.0f, 0.0f);

			// Stretch texture over grid.
			meshData.Vertices[static_cast<uint64_t>(i) * n + j].TexC.x = j * du;
			meshData.Vertices[static_cast<uint64_t>(i) * n + j].TexC.y = i * dv;
		}
	}
	...
	meshData.Indices32.resize(static_cast<uint64_t>(faceCount) * 3); // 3 indices per face
	...
	for (uint32 i = 0; i < m - 1; ++i)
	{
		for (uint32 j = 0; j < n - 1; ++j)
		{
			...
			meshData.Indices32[static_cast<uint64_t>(k) + 1] = i * n + j + 1;
			meshData.Indices32[static_cast<uint64_t>(k) + 2] = (i + 1) * n + j;

			meshData.Indices32[static_cast<uint64_t>(k) + 3] = (i + 1) * n + j;
			meshData.Indices32[static_cast<uint64_t>(k) + 4] = i * n + j + 1;
			meshData.Indices32[static_cast<uint64_t>(k) + 5] = (i + 1) * n + j + 1;
...
GeometryGenerator::MeshData GeometryGenerator::CreateQuad(float x, float y, float w, float h, float depth) noexcept
{
	MeshData meshData;
